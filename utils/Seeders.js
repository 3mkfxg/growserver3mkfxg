...
exports.createSeeders = createSeeders;
...
function validateRange(min, max, paramName = 'range') {
...
function validateSafeInteger(value, paramName) {
...
class Seeders {
    ...
    stringToSeed(str) {
        if (!str.length)
            return 3000;
        let hash = str.split('').reduce((acc, char) => ((acc << 5) - acc) + char.charCodeAt(0) | 0, 0);
     ...
     * ```
     */
    alphanumeric(length, charset = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') {
     ...
     * ```
     */
    noise(x, amplitude = 1, max = 1, min = -1, frequency = 0.01, octaves = 1) {
 ...
 * ```
 */
function createSeeders(seed, salt = '') {
